...
onMounted(async () => {
  const { data } = await client.get(`/categorias/${route.params.id}`)
  form.value.nome = data.nome
  form.value.ordem = data.ordem
...
async function submit() {
  ...
  loading.value = true
  try {
    await client.patch(`/categorias/${route.params.id}`, form.value)
...
<template>
  <AuthenticatedLayout>
    <template #header><h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100">Editar Categoria de Função</h2></template>
    <Card>
