...
async function destroy(id: number) {
  if (!confirm('Confirma a exclusão deste celebrante?')) return
  try {
    await client.delete(`/celebrantes/${id}`)
    celebrantes.value = celebrantes.value.filter((c) => c.id !== id)
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex flex-wrap justify-between items-center gap-3">
        <h2 class="font-semibold text-xl text-gray-800">Celebrantes</h2>
        <RouterLink v-if="auth.isStaff" to="/celebrantes/criar" class="inline-flex items-center px-4 py-2 bg-gray-800 text-white text-xs font-semibold uppercase rounded-md hover:bg-gray-700">
          Novo Celebrante
        </RouterLink>
      </div>
    </template>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="c in celebrantes" :key="c.id">
            ...
            <td class="px-6 py-4 text-sm text-gray-500">{{ c.telefone ?? c.email ?? '—' }}</td>
            ...
            <td v-if="auth.isStaff" class="px-6 py-4 text-right space-x-3 whitespace-nowrap">
              <RouterLink :to="`/celebrantes/${c.id}/editar`" class="text-indigo-600 hover:text-indigo-900 text-sm">Editar</RouterLink>
              <button @click="destroy(c.id)" class="text-red-600 hover:text-red-900 text-sm">Excluir</button>
            </td>
          </tr>
          <tr v-if="celebrantes.length === 0">
            <td colspan="4" class="px-6 py-8 text-center text-gray-500">Nenhum celebrante cadastrado.</td>
          </tr>
        </tbody>
      </table>
      </div>
