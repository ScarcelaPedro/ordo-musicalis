...
async function submit() {
  ...
  loading.value = true
  try {
    await client.post('/comunidades', form.value)
    flash.set('success', 'Comunidade criada com sucesso!')
    router.push('/comunidades')
  } catch (e: any) {
    flash.set('error', e.response?.data?.message ?? 'Erro ao criar comunidade')
  } finally {
    loading.value = false
  }
}
...
<template>
  <AuthenticatedLayout>
    <template #header><h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100">Nova Comunidade</h2></template>
    <Card>
