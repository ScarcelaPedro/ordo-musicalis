...
async function destroy(id: number) {
  if (!confirm('Confirma a exclusão desta comunidade?')) return
  try {
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex flex-wrap justify-between items-center gap-3">
        <h2 class="font-semibold text-xl text-gray-800">Comunidades</h2>
        <RouterLink v-if="auth.isStaff" to="/comunidades/criar" class="inline-flex items-center px-4 py-2 bg-gray-800 text-white text-xs font-semibold uppercase rounded-md hover:bg-gray-700">
          Nova Comunidade
        </RouterLink>
      </div>
    </template>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="c in comunidades" :key="c.id">
            ...
            <td class="px-6 py-4 text-sm text-gray-500">{{ c.endereco ?? '—' }}</td>
            ...
            <td v-if="auth.isStaff" class="px-6 py-4 text-right space-x-3 whitespace-nowrap">
              <RouterLink :to="`/comunidades/${c.id}/editar`" class="text-indigo-600 hover:text-indigo-900 text-sm">Editar</RouterLink>
              <button @click="destroy(c.id)" class="text-red-600 hover:text-red-900 text-sm">Excluir</button>
            </td>
          </tr>
          <tr v-if="comunidades.length === 0">
            <td colspan="4" class="px-6 py-8 text-center text-gray-500">Nenhuma comunidade cadastrada.</td>
          </tr>
        </tbody>
      </table>
      </div>
